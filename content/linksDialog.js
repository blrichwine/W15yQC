...
function fnDelayedInit() {
  window.setTimeout(function(){blr.W15yQC.LinksDialog.init(window);}, 0);
}
...
blr.W15yQC.LinksDialog = {
  ...
  fnPopulateTree: function (aDocumentsList, aLinksList, bDontHideCols) {
    var tbc, bHasRole=false, bHasStateDescription=false, bHasTarget=false, bHasTitle=false, i, ak, ch, treecell, treeitem, treerow, textbox, order;
      ...
      if (tbc != null) {
        ...
        if (bDontHideCols!=true) {
            for (i = 0; i < aLinksList.length; i++) {
              ak = aLinksList[i];
              if (ak.title != null) bHasTitle = true;
  ...
  cleanup: function () {
    ...
    blr.W15yQC.LinksDialog.sortColumns = null;
  },
  ...
  updateNotesField: function (bHighlightElement) {
    ...
    ak=blr.W15yQC.LinksDialog.aLinksList[selectedIndex];
    ...
    textbox.value = blr.W15yQC.fnJoin(textbox.value, 'BaseURI: ' + blr.W15yQC.LinksDialog.aDocumentsList[ak.ownerDocumentNumber - 1].URL, "\n");
